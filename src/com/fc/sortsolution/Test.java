...
//        arr[i] = arr[i] ^ arr[j];
    ...
    public static void main(String[] args) {
        Test test = new Test();
        int[] arr = {4, 6, 8, 5, 9, 2, 3, 1, 6, 7, 11, 20, 3};
...
//        }
    ...
    private int[] heapSort(int[] targetArr) {
    ...
    private int[] quickSortOper(int[] arr, int left, int right) {
        if (left < right) {
            int partition = partiton(arr, left, right);
            quickSortOper(arr, left, partition - 1);
            quickSortOper(arr, partition + 1, right);
        }
        return arr;
    }
